...
let example_module_path = "Bindoj_test_common_typedesc_examples.Ex18"
  ...
  let make_decl with_doc
    ...
    variant_decl "optional_variant" [
      ...
      variant_constructor "Tuple_like_alias" (`tuple_like [
        variant_argument_nested ~codec:(open_ "Ex17") (decl (module Ex17))
      ...
      ]) ~configs:(Json_config.tuple_style (`obj `default) :: constructor_configs)
        ~doc:(doc "Tuple_like_obj constructor");
      variant_constructor "Tuple_like_spreading" (`tuple_like [
        variant_argument_nested ~codec:(open_ "Ex10") (decl (module Ex10))
          ~configs:[
            Json_config.nested_field_style `spreading;
          ];
      ]) ~configs:constructor_configs ~doc:(doc "tuple_like_spreading constructor");
      variant_constructor "Inline_record" (`inline_record [
        record_field_nested "int_opt" ~codec:(open_ "Ex17") (decl (module Ex17))
      ...
      variant_constructor "Inline_record_spreading" (`inline_record [
        ...
        record_field_nested "xy_opt" ~codec:(open_ "Ex10") (decl (module Ex10))
          ~configs:[
            Json_config.nested_field_style `spreading;
          ]
      ...
      variant_constructor "Reused_inline_record" (`reused_inline_record (decl (module Ex10)))
        ~configs:(
          Ts_config.reused_variant_inline_record_style `intersection_type
          :: constructor_configs)
        ~doc:(doc "Reused_inline_record constructor");
    ] ~configs:variant_configs ~doc:(doc "definition of optional_variant type")
end
...
let fwrt : (unit, unit, unit) ts_fwrt_decl =
  let optional_variant = "optional_variant" in
  optional_variant, Util.FwrtTypeEnv.(
    init
    |> bind_alias "int_opt" cty_int_opt
    |> bind_object "xy_opt" [
      field "x_opt" cty_int_opt;
      field "y_opt" cty_int_opt; ]
    |> bind_object optional_variant ~configs:variant_configs []
    |> bind_constructor ~parent:optional_variant "Tuple_like" ~args:[
        variant_argument cty_int_opt
      ] ~configs:constructor_configs
    |> bind_constructor ~parent:optional_variant "Tuple_like_alias" ~args:[
        variant_argument_nested ~codec:(`open_ "Ex17_gen") "int_opt"
      ] ~configs:constructor_configs
    |> bind_constructor ~parent:optional_variant "Tuple_like_obj" ~args:[
      variant_argument cty_int_opt;
      variant_argument_nested ~codec:(`open_ "Ex17_gen") "int_opt";
    ] ~configs:(Json_config.tuple_style (`obj `default) :: constructor_configs)
    |> bind_constructor ~parent:optional_variant "Tuple_like_spreading" ~args:[
        variant_argument_nested ~codec:(`open_ "Ex10_gen") "xy_opt"
          ~configs:[
            Json_config.nested_field_style `spreading;
          ]
      ] ~configs:constructor_configs
    |> bind_constructor ~parent:optional_variant "Inline_record" ~fields:[
        field_nested ~codec:(`open_ "Ex17_gen") "int_opt" "int_opt";
        field "x_opt" cty_int_opt;
        field "y_opt" cty_int_opt;
        field "objtuple" cty_tuple;
      ] ~configs:constructor_configs
    |> bind_constructor ~parent:optional_variant "Inline_record_spreading" ~fields:[
        field_nested ~codec:(`open_ "Ex17_gen") "int_opt" "int_opt";
        field_nested ~codec:(`open_ "Ex10_gen") "xy_opt" "xy_opt"
          ~configs:[
            Json_config.nested_field_style `spreading;
          ];
      ] ~configs:constructor_configs
    |> bind_constructor ~parent:optional_variant "Reused_inline_record"
        ~annot_kc:(Some (Tfcki_reused_variant_inline_record Ex10.decl))
        ~fields:[
          field "x_opt" cty_int_opt;
          field "y_opt" cty_int_opt;
        ]
  ...
  Some [
    `type_alias_declaration
      { tsa_modifiers = [`export];
        tsa_name = "OptionalVariant";
        tsa_type_parameters = [];
        tsa_type_desc = `union (List.map snd optional_variant); };
    Util.Ts_ast.case_analyzer "OptionalVariant" "analyzeOptionalVariant" options optional_variant;
  ]
