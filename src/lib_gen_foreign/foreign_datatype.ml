...
module Format = struct
  type ('ann_d, 'ann_f, 'ann_ko, 'ann_ka, 'ann_kc) fwrt_desc = {
    fd_name : string;
    fd_parent : string option;
    fd_kind : ('ann_f, 'ann_ko, 'ann_ka, 'ann_kc) fwrt_desc_kind;
    fd_annot : 'ann_d;
    fd_doc : doc;
  } [@@deriving show,eq]

  and ('ann_f, 'ann_ko, 'ann_ka, 'ann_kc) fwrt_desc_kind =
    | Fwrt_object of {
...
let pp_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc ppf =
  fwrt_desc_to_format
  &> Format.pp_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc ppf

let show_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc =
  fwrt_desc_to_format
  &> Format.show_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc

let equal_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc d1 d2 =
  Format.equal_fwrt_desc pp_ann_d pp_ann_f pp_ann_ko pp_ann_ka pp_ann_kc
    (fwrt_desc_to_format d1) (fwrt_desc_to_format d2)

type ('ann_d, 'ann_f, 'ann_k) fwrt_type_env = ('ann_d, 'ann_f, 'ann_k) fwrt_desc StringMap.t
constraint 'ann_k = _*_*_
...
module FwrtTypeEnv = struct
  ...
  let init : ('ann_d, 'ann_f, 'ann_k) t = StringMap.empty
  ...
  let bind_object ?doc ?parent ?(configs = Configs.empty) ~annot_d ~annot_ko name fields (env: ('ann_d, 'ann_f, 'ann_ko*'ann_ka*'ann_kc) t)
    : ('ann_d, 'ann_f, 'ann_ko*'ann_ka*'ann_kc) t =
    let kind =
      Fwrt_object {
  ...
  let lookup : string -> ('ann_d, 'ann_f, 'ann_k) t -> ('ann_d, 'ann_f, 'ann_k) fwrt_desc =
    StringMap.find

  let lookup_opt : string -> ('ann_d, 'ann_f, 'ann_k) t -> ('ann_d, 'ann_f, 'ann_k) fwrt_desc option  =
    StringMap.find_opt

  let bindings = StringMap.bindings

  let annotate_kind ann_f = function
  ...
  let annotate
    ...
    |> StringMap.mapi (fun name desc ->
      let ann_d, ann_f =
        if name = typ then
          ann_d_of_type, ann_f_of_typ
        else
          ann_d_of_other, ann_f_of_other
      in
      { desc with
          fd_kind = annotate_kind ann_f desc.fd_kind;
          fd_annot = ann_d; })
...
module FwrtTypeEnv'(D : sig
  ...
  let lookup : string -> t -> (D.annot_d, D.annot_f, D.annot_ko*D.annot_ka*D.annot_kc) fwrt_desc =
    FwrtTypeEnv.lookup

  let lookup_opt : string -> t -> (D.annot_d, D.annot_f, D.annot_ko*D.annot_ka*D.annot_kc) fwrt_desc option =
    FwrtTypeEnv.lookup_opt

  let bindings : t -> (string * (D.annot_d, D.annot_f, D.annot_ko*D.annot_ka*D.annot_kc) fwrt_desc) list =
...
let pp_fwrt_decl :
  ...
  -> unit
  = fun ann_d ann_f ann_ko ann_ka ann_kc ppf (name, env) ->
  let fd = FwrtTypeEnv.lookup name env in
  pp_fwrt_desc ann_d ann_f ann_ko ann_ka ann_kc ppf fd
...
let show_fwrt_decl :
  ...
  -> ('ann_d, 'ann_f, 'ann_ko*'ann_ka*'ann_kc) fwrt_decl
  -> string
  = fun ann_d ann_f ann_ko ann_ka ann_kc (name, env) ->
  let fd = FwrtTypeEnv.lookup name env in
  show_fwrt_desc ann_d ann_f ann_ko ann_ka ann_kc fd
...
let equal_fwrt_decl :
  ...
  -> ('ann_d, 'ann_f, 'ann_ko*'ann_ka*'ann_kc) fwrt_decl
  -> bool
  = fun ann_d ann_f ann_ko ann_ka ann_kc (name1, env1) (name2, env2) ->
  let fd1 = FwrtTypeEnv.lookup name1 env1 in
  let fd2 = FwrtTypeEnv.lookup name2 env2 in
  equal_fwrt_desc ann_d ann_f ann_ko ann_ka ann_kc fd1 fd2
...
let fwrt_decl_of_type_decl : type_decl -> (unit, unit, unit*unit*unit) fwrt_decl =
  fun decl ->
