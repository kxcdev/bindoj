...
import { ByteSource, ByteSource$PRIME$, GeneratedBridge } from "../generated/objintf";
...
async function main() {
  function assertEqual<T>(expected: T, actual: T) {
    if (expected !== actual) throw `${actual} is not equal to ${expected}!`;
  }
  ...
  // example_remote_logging
  usage.usage()?.example_remote_logging();
  assertEqual("hello from Usage1", output);

  // example_stdout
  usage.usage()?.example_stdout();
  assertEqual("hello", new TextDecoder().decode(mockStdout.buf));

  // example_pass_endemic_object
  ...
  assertEqual("hello?", new TextDecoder().decode(mockFiles["text.txt"].buf));
